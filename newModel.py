...
class MDTA(nn.Module):
    def __init__(self, channels, num_heads):
        ...
        self.relu = nn.ReLU()
        self.sparse_attention_weight = nn.Parameter(torch.ones(1, num_heads, 1, 1))
    ...
    def forward(self, x):
        ...
        amp_q = amp_q.reshape(b, self.num_heads, -1, h * w)
        amp_k = amp_k.reshape(b, self.num_heads, -1, h * w)
        q = q.reshape(b, self.num_heads, -1, h * w)
        k = k.reshape(b, self.num_heads, -1, h * w)
        v = v.reshape(b, self.num_heads, -1, h * w)
        amp_q, amp_k = F.normalize(amp_q, dim=-1), F.normalize(amp_k, dim=-1)
        q, k = F.normalize(q, dim=-1), F.normalize(k, dim=-1)
        sparse_attn = torch.matmul(amp_q, amp_k.transpose(-2, -1).contiguous())
        sparse_attn = self.softmax(sparse_attn)
        sparse_attn = self.relu(sparse_attn)**2
        
        sparse_attn = torch.matmul(sparse_attn, v)
        sparse_attn = sparse_attn.reshape(b, -1, h, w)
        attn = torch.softmax(torch.matmul(q, k.transpose(-2, -1).contiguous()) * self.temperature, dim=-1)
        out = self.project_out(torch.matmul(attn, v).reshape(b, -1, h, w) + sparse_attn * self.sparse_attention_weight)
        return out
...
class PhaseNewModel(nn.Module):
    def __init__(self, num_blocks=[2, 3, 3, 4], num_heads=[1, 2, 4, 8], channels=[16, 32, 64, 128], num_refinement=4,
        ...
        super(PhaseNewModel, self).__init__()
        # self.sig=nn.Sigmoid()
        self.attention = nn.ModuleList([ECA(num_ch) for num_ch in ch])
       
        self.embed_conv_rgb = nn.Sequential([nn.Conv2d(3, 8, kernel_size=3, padding=1, bias=False),
                                             nn.Conv2d(8, channels[0], kernel_size=3, padding=1, bias=False),
                                             nn.Sigmoid()])
